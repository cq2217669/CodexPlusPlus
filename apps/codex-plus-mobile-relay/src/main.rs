...
async fn main() -> anyhow::Result<()> {
    ...
    println!("Xuan++ mobile relay listening on ws://{local_addr}");
    println!(
...
<body>
<div class="app">
  <header class="topbar"><strong class="title">Xuan++</strong><span id="status" class="status">待连接</span></header>
  <main class="layout">
...
async function ensureAppServer() {
  ...
  await rpcRaw("initialize", { clientInfo: { name: "Xuan++ Mobile Relay", version: "1.0.0" }, capabilities: { experimentalApi: true } });
}

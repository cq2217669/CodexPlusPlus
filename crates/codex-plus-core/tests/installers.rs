...
fn windows_entrypoint_plan_contains_silent_and_manager_entrypoints() {
    ...
    assert!(plan.silent_shortcut.ends_with("Xuan++.lnk"));
    assert!(plan.manager_shortcut.ends_with("Xuan++ 管理工具.lnk"));
...
fn macos_bundle_metadata_contains_silent_and_manager_apps() {
    let options = InstallOptions {
        ...
        launcher_path: Some("/opt/Xuan++/codex-plus-plus".into()),
        manager_path: Some("/opt/Xuan++/codex-plus-plus-manager".into()),
        remove_owned_data: false,
    };
    ...
    assert!(silent.app_path.ends_with("Xuan++.app"));
    assert!(manager.app_path.ends_with("Xuan++ 管理工具.app"));
    assert!(silent.info_plist.contains("<string>Xuan++</string>"));
    assert!(
        manager
            .info_plist
...
fn companion_binary_path_resolves_macos_silent_app_next_to_manager_app() {
    let manager_exe = std::path::Path::new(
        "/Applications/Xuan++ 管理工具.app/Contents/MacOS/CodexPlusPlusManager",
    );
    ...
    assert_eq!(
        companion,
        std::path::PathBuf::from("/Applications/Xuan++.app/Contents/MacOS/CodexPlusPlus")
    );
    assert_ne!(
        companion,
        std::path::PathBuf::from(
            "/Applications/Xuan++ 管理工具.app/Contents/MacOS/codex-plus-plus"
        )
    );
}
...
fn companion_binary_path_resolves_macos_manager_app_next_to_silent_app() {
    let silent_exe = std::path::Path::new("/Applications/Xuan++.app/Contents/MacOS/CodexPlusPlus");
...
fn macos_companion_launch_uses_bundle_ids_from_app_translocation() {
    let manager_exe = std::path::Path::new(
        "/private/var/folders/x/AppTranslocation/manager-id/d/Xuan++ 管理工具.app/Contents/MacOS/CodexPlusPlusManager",
    );
    let silent_exe = std::path::Path::new(
        "/private/var/folders/x/AppTranslocation/silent-id/d/Xuan++.app/Contents/MacOS/CodexPlusPlus",
    );
...
fn macos_companion_path_falls_back_to_workspace_release_launcher() {
    ...
    let bundle_exe = root.path().join(
        "target/release/bundle/macos/Xuan++ Manager.app/Contents/MacOS/codex-plus-plus-manager",
    );
...
fn macos_bundle_does_not_wrap_the_bundle_executable_in_itself() {
    let options = InstallOptions {
        install_root: Some("/Applications".into()),
        launcher_path: Some("/Applications/Xuan++.app/Contents/MacOS/CodexPlusPlus".into()),
        manager_path: Some(
            "/Applications/Xuan++ 管理工具.app/Contents/MacOS/CodexPlusPlusManager".into(),
        ),
        remove_owned_data: false,
    };
    ...
    assert_eq!(
        ...
        Some(std::path::PathBuf::from(
            "/Applications/Xuan++.app/Contents/MacOS/CodexPlusPlus"
        ))
    );
    assert_eq!(

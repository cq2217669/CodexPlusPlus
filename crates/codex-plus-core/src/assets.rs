...
const RELAY_BALANCE_SCRIPT: &str = include_str!("../../../assets/inject/relay-balance-inject.js");
const WORKSPACE_SEARCH_SCRIPT: &str =
    include_str!("../../../assets/inject/workspace-search-inject.js");
...
pub const INJECT_ASSET_REVISION: &str = env!("CODEX_PLUS_INJECT_ASSET_REVISION");
...
pub fn relay_balance_script() -> &'static str {
    RELAY_BALANCE_SCRIPT
}

pub fn workspace_search_script() -> &'static str {
    WORKSPACE_SEARCH_SCRIPT
}
...
pub fn injection_script_with_settings(helper_port: u16, settings: &BackendSettings) -> String {
    ...
    format!(
        ...
        serde_json::to_string(INJECT_ASSET_REVISION)
            .expect("injection asset revision should serialize"),
...
mod tests {
    ...
    #[test]
    fn injection_script_exposes_the_compiled_asset_revision() {
        let script = injection_script(57321);

        assert!(script.contains("window.__CODEX_PLUS_INJECT_ASSET_REVISION__"));
        assert!(script.contains(INJECT_ASSET_REVISION));
    }
